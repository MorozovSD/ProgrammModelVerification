...
from languade.expressions import Brace, BinaryExpression, UnaryExpression, IndexOrParameter, CallOrIndexer
from languade.function import Function, FuncSignature
from languade.source import Source
from languade.statements import NewIdentifier, If, While, Break, Expression, Assignment, Statements
from languade.variables import Argument, Array, Identifier, Literal
...
def p_funcSignature(p):
    ...
    if len(p) == 7:
        ...
        p[0] = FuncSignature(name=p[1].name, args=p[3], pos=set_pos(p, 1),
                             func_type=p[7], children=[identifier, argDefs, typeRef])
    else:
...
def p_var(p):
    ...
    p[0] = NewIdentifier(identifiers=p[2], type=p[4], pos=set_pos(p, 1), children=[dim])
...
def p_do(p):
    ...
    p[0] = While(expr=p[2], do_stmt=p[5], loop_type=p[3], pos=set_pos(p, 2), children=[_do, _loop])
...
def p_expression(p):
    ...
    p[0] = Expression(expr=p[1], pos=set_pos(p, 1), children=[expression])
...
def p_assignment(p):
    ...
    p[0] = Assignment(p[1], p[3], pos=set_pos(p, 1), children=[assignment])
...
def p_callOrIndexer(p):
    ...
    p[0] = CallOrIndexer(expr=p[3], parameters=parameters, pos=set_pos(p, 1), children=[expr, callOrIndexer])
